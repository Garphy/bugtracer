...
$foot_out = '';
$type = $_GET['type'] ?? 'all';
$type = $type !== 'all' ? intval($type) : 'all';
$page = $_GET['page'] ?? 1;
$page = intval($page);
$order = $_GET['order'] ?? 'id';
$order = filter($order);
$filter = $_GET['status'] ?? $_G['defaultFilter'];
//search
if (isset($_GET['search'])) {
    $sql = "SELECT i.*, t.name FROM `items` AS i LEFT JOIN `types` AS t ON i.type = t.tid WHERE t.pid = ?";
    $params = [$thisPid];
    $types = ['i'];
    if ($_GET['search'] == 'alll') { //all
        header('Location:./report.php');
        exit;
    } elseif (preg_match('/^\d+,?/', $_GET['search'])) { //by id
        $ids = explode(',', $_GET['search']);
        $ids = array_filter(array_map('intval', $ids));
        if (!empty($ids)) {
            $sql .= " AND i.id IN (" . implode(',', array_fill(0, count($ids), '?')) . ")";
            $params = array_merge($params, $ids);
            $types = array_merge($types, array_fill(0, count($ids), 'i'));
        }
    ...
    } elseif (preg_match('/^\((\d+)\)$/', $_GET['search'], $match)) { //by creator
        $user = $match[1];
        $sql .= " AND i.creator = ?";
        $params[] = $user;
        $types[] = 'i';
        $sql .= ' AND i.status IN (' . implode(', ', array_fill(0, count($filter), '?')) . ')';
        $params = array_merge($params, $filter);
        $types = array_merge($types, array_fill(0, count($filter), 'i'));
    } elseif (preg_match('/^{(\d+)}$/', $_GET['search'], $match)) { //by assignto
        $user = $match[1];
        $sql .= " AND i.assignto = ?";
        $params[] = $user;
        $types[] = 'i';
        ...
        $params = array_merge($params, $filter);
        $types = array_merge($types, array_fill(0, count($filter), 'i'));
    } elseif (preg_match('/^!{(\d+)}$/', $_GET['search'], $match)) { //assignto excluded
        $user = $match[1];
        $sql .= " AND i.assignto != ?";
        $params[] = $user;
        $types[] = 'i';
        ...
        $params = array_merge($params, $filter);
        $types = array_merge($types, array_fill(0, count($filter), 'i'));
    } elseif (preg_match('/^{(\d{4}-\d+-\d+)~(\d{4}-\d+-\d+)}$/', $_GET['search'], $match)) { //by changetime
        $from = $match[1];
        $to = $match[2];
        $t0 = strtotime($from);
        $t1 = strtotime($to);
        $sql .= " AND i.changetime >= ? AND i.changetime < ?";
        $params[] = $t0;
        $params[] = $t1;
        $types[] = 'i';
        $types[] = 'i';
...
//list
    ...
    if (!empty($filter)) {
        $cond[] = 'i.status IN (' . implode(', ', array_fill(0, count($filter), '?')) . ')';
        $params = array_merge($params, $filter);
        $types = array_merge($types, array_fill(0, count($filter), 'i'));
    }
    if ($_G['mode'] == 'coder') {
...
if (mysqli_num_rows($result)) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $row['content'] = (strlen($row['content']) > $titleMaxLen) ? 
            makeTags(mb_substr($row['content'], 0, $titleMaxLen, 'utf-8')) . ' <a class="more" href="javascript:void(0);">[...]</a>'
            :
            makeTags($row['content']);
        $row['attach'] = !empty($row['files']);
        $row['assigned'] = (
...
//total items
if (!isset($shown_items)) $shown_items = mysqli_num_rows($result);
...
<div class="top">
    ...
    <div class="mode">
        <?php
            $modes = [
                'admin' => ['name' => '管理模式', 'target' => 'Debug模式', 'link' => 'debugmode'],
                'coder' => ['name' => 'Debug模式', 'target' => '管理模式', 'link' => 'adminmode']
            ];
            $mode = $modes[$_G['mode']];
        ?>
...
<div class="content">
    ...
    <ul id="items">
    <!--<li type="1" id="bug_1"><div><b>1</b> <em status="3" class="fixed"><div class="flagger"></div>
    ...
    <div id="flaggerLayer">
        ...
        <ul>
            ...
            foreach ($_G['status'] as $n => $status) {
                if ($_G['role'] == 'coder' && !in_array($status, ['fixed', 'part_fixed', 'wont_fix'])) continue; //coder禁止部分功能
