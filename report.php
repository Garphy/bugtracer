...
if (mysqli_num_rows($result)) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $row['content'] = makeTags($row['content']);
        $urls = [];
        if (!empty($row['files'])) {
...
//stats
$stats = [];
$stats['all'] = ['total' => 0, 'fixed' => 0, 'key' => 0, 'name' => $userNames['all']];
...
if (mysqli_num_rows($result)) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (in_array($row['status'], [1, 2, 3])) { 
            $stats[$uid]['total'] = ($stats[$uid]['total'] ?? 0) + $row['sum'];
            $stats['all']['total'] += $row['sum'];
        //已解决bug：04
        } elseif (in_array($row['status'], [0, 4])) {
            $stats[$uid]['fixed'] = ($stats[$uid]['fixed'] ?? 0) + $row['sum'];
            $stats['all']['fixed'] += $row['sum'];
        }
        //关键bug：2
        if ($row['status'] == 2) {
            $stats[$uid]['key'] = ($stats[$uid]['key'] ?? 0) + $row['sum'];
...
<div class="content">
    <div id="filter">
        <input type="text" id="searchBug" value="<?php echo $_GET['search'] ?? 'Bug搜索/ID查询' ?>" />
